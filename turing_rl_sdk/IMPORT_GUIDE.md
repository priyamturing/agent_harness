...
from turing_rl_sdk.agents.core import ClaudeAgent, GPTAgent, GeminiAgent, GrokAgent

# Task and Result structures
from turing_rl_sdk.agents.tasks import Task, Result, AgentResponse

# MCP configuration
from turing_rl_sdk.agents.mcp import MCPConfig, MCPClientManager

# Runtime components
from turing_rl_sdk.agents.runtime import RunContext, RunObserver

# Test harness
from turing_rl_sdk.harness import TestHarness, TestHarnessConfig, create_agent

# Verifiers
from turing_rl_sdk.harness.verifiers import Verifier, DatabaseVerifier, VerifierResult
```
...
```python
from turing_rl_sdk.agents.core import ClaudeAgent
from turing_rl_sdk.agents.tasks import Task
...
**Top-level (quick):**
```python
from turing_rl_sdk import Agent, AgentResponse, RunContext, RunObserver
...
### Top-Level Exports (`from turing_rl_sdk import ...`)
...
```json
{
  "python.analysis.extraPaths": [
    "${workspaceFolder}/packages/turing_rl_sdk/src"
  ],
...
```bash
# Replace imports
sed -i 's/from turing_rl_sdk import ClaudeAgent/from turing_rl_sdk.agents.core import ClaudeAgent/g' *.py
```

**Explicit → Top-level:**
```bash
# Replace imports (reverse)
sed -i 's/from turing_rl_sdk.agents.core import \(.*\)/from turing_rl_sdk import \1/g' *.py
```
