...
from ..runtime import RunContext, StatusLevel, MessageRole
...
class Agent(ABC):
    ...
    def __init__(
        ...
        Args:
            ...
            tool_call_limit (Optional[int]): Maximum number of tool calls allowed 
                before stopping execution. If None, uses DEFAULT_TOOL_CALL_LIMIT.
        ...
        self.tool_call_limit = tool_call_limit if tool_call_limit is not None else DEFAULT_TOOL_CALL_LIMIT
        self.timeout = timeout if timeout is not None else DEFAULT_LLM_TIMEOUT_SECONDS
    ...
    async def run(
        ...
        *,
        ...
        Raises:
            Exception: Initialization failures (MCP connection, LLM setup) can
                propagate. LLM errors during execution are caught and returned
    ...
    async def initialize(self, task: Task, run_context: RunContext) -> None:
        ...
        Raises:
            Exception: MCP server connection failures, invalid configuration,
                or LLM initialization errors from _build_llm().
    ...
    async def call_tools(self, tool_calls: list[ToolCall]) -> list[ToolResult]:
        ...
        Note:
            ...
            - Telemetry notifications are sent to run_context if available. When using
              the low-level API, set self._run_context before calling this method to
              enable observability. The high-level run() API sets this automatically.
        """
        ...
        for tc in tool_calls:
            ...
            try:
                ...
                if hasattr(tool, "ainvoke"):
                    output = await tool.ainvoke(tc.arguments, config=tool_config)  # type: ignore[union-attr]
                else:
                    loop = asyncio.get_running_loop()
                    output = await loop.run_in_executor(None, lambda: tool.invoke(tc.arguments, config=tool_config))  # type: ignore[union-attr]
    ...
    async def cleanup(self) -> None:
        ...
        Raises:
            Exception: MCP or RunContext cleanup failures can propagate.
                LLM cleanup errors are silently caught.
        """
        ...
        if self._llm:
            if hasattr(self._llm, 'aclose'):
                ...
                logger.debug(
                    f"No cleanup method found for LLM type {type(self._llm).__name__}. "
                    "This is usually safe but may leave connections open."
                )
    ...
    async def _execute_loop(self, max_steps: int, run_context: RunContext) -> Result:
        ...
        Raises:
            TypeError: If a tool returns non-JSON-serializable data (from call_tools).
            RuntimeError: If tool_calls/results mismatch (from format_tool_results).
        
        Note:
            LLM errors from get_response() are caught and returned in Result.error.
        """
        ...
        for msg in messages:
            if isinstance(msg, SystemMessage):
                ...
                await run_context.notify_message(MessageRole.SYSTEM, content)
            elif isinstance(msg, HumanMessage):
                content = self._extract_text_content(msg.content)
                await run_context.notify_message(MessageRole.USER, content)

        for step in range(max_steps):
            await run_context.notify_status(f"Step {step + 1}/{max_steps}", StatusLevel.INFO)

            try:
                response, ai_message = await self.get_response(messages)
            except Exception as exc:
                await run_context.notify_status(f"Model error: {exc!r}", StatusLevel.ERROR)
                return Result(
                    success=False,
                    messages=messages,
            ...
            if (
                ...
                await run_context.notify_status(error_msg, StatusLevel.WARNING)
                return Result(
                    success=False,
                    messages=messages,
                    metadata={
            ...
            if response.done and not response.tool_calls:
                await run_context.notify_status("Agent completed", StatusLevel.INFO)
        ...
        await run_context.notify_status("Max steps reached", StatusLevel.WARNING)
    ...
    def _build_llm(self) -> Union[BaseChatModel, Runnable]:
        ...
        Raises:
            Exception: API key errors, invalid configuration, network issues,
                or provider-specific initialization failures.
        """
        ...
