# MCP Benchmark SDK - Initial Release
...
This PR introduces the **MCP Benchmark SDK**, a comprehensive Python framework for building and running LLM agent benchmarks against MCP (Model Context Protocol) servers. The SDK provides a **harness-first** design to systematically evaluate LLM agents across multiple models, scenarios, and verification criteria.
...
- **46 files changed**
- **7,849 lines added**
- **New Package**: `mcp-benchmark-sdk`
- **2 Packages Total**: SDK (core) + CLI (user interface)
...
results = await harness.run(
    models=["gpt-5-high", "claude-sonnet-4-5", "gemini-2.5-pro"],
...
|----------|-------------|---------------|----------|
| **OpenAI** | `GPTAgent` | `gpt-5-high` | High reasoning effort, encrypted reasoning |
| **Anthropic** | `ClaudeAgent` | `claude-sonnet-4-5` | Extended thinking, 10k token budget |
...
- ✅ Automatic retry logic with exponential backoff (configurable: 3 retries default)
...
```python
from mcp_benchmark_sdk import ClaudeAgent, GPTAgent, GeminiAgent, GrokAgent

# Use defaults (with reasoning enabled)
agent = ClaudeAgent()  # claude-sonnet-4-5 with thinking
...
```python
from mcp_benchmark_sdk import RunObserver, RunContext
...
```python
from mcp_benchmark_sdk import configure_langsmith, with_tracing, create_traced_agent
...
# Create traced agent
agent = create_traced_agent("gpt-5-high")
...
### Core SDK Files (`src/mcp_benchmark_sdk/`)

#### Agent System (`agents/`)
- **`core/base.py`** (613 lines) - Abstract agent class with multi-level API (high/mid/low)
- **`core/claude.py`** (235 lines) - Anthropic Claude with extended thinking
- **`core/gpt.py`** (197 lines) - OpenAI GPT with reasoning effort
- **`core/gemini.py`** (200 lines) - Google Gemini with thinking budget
- **`core/grok.py`** (165 lines) - xAI Grok with reasoning support
- **`constants.py`** (28 lines) - Global constants and defaults

#### Task Management (`tasks/`)
- **`task.py`** (64 lines) - Task definition and configuration
- **`result.py`** (226 lines) - Result models with conversation history export
...
- **394 lines** - LangSmith/Langfuse integration, token tracking, trace management

#### Harness System (`harness/`)
- **`orchestrator.py`** (406 lines) - TestHarness class, concurrent execution, result aggregation
- **`loader.py`** (291 lines) - Load scenarios from JSON with validation
- **`scenario.py`** (121 lines) - Scenario data models (frozen dataclasses)
- **`agent_factory.py`** (174 lines) - Agent creation with auto-detection
- **`verifiers/base.py`** (47 lines) - Verifier interface
- **`verifiers/database.py`** (263 lines) - SQL-based verification with comparison logic
...
#### Documentation Files
- **`README.md`** (827 lines) - Complete SDK reference with:
...
- **`QUICKSTART.md`** (202 lines) - Ultra-fast 30-second start guide
- **`IMPORT_GUIDE.md`** (319 lines) - Import patterns and module structure
- **`harness/README.md`** (184 lines) - Test harness specific documentation

#### Examples (`examples/`)
- **`simple_harness_example.py`** (107 lines) - Basic harness usage with console observer
- **`save_artifacts_example.py`** (78 lines) - Exporting results to JSON with conversation history
- **`simple_task.json`** (29 lines) - Example task definition
- **`SDK_USAGE_GUIDE.ipynb`** (988 lines) - Step-by-step interactive tutorial
- **`sdk_usage_showcase.ipynb`** (1,325 lines) - Complete showcase with all patterns

#### Architecture & Analysis
- **`SDK_ARCHITECTURE.md`** (1,313 lines) - Detailed architecture documentation with:
  - Component layer breakdown
  - Sequence diagrams for execution flows
  - Design decisions and rationale
  - Code examples for each pattern
  
- **`SDK_CODE_ANALYSIS.md`** (906 lines) - Historical code review report
- **`REFACTORING_SUMMARY.md`** (213 lines) - Migration guide from CLI-only to SDK+CLI
...
```python
from mcp_benchmark_sdk import ClaudeAgent, Task, MCPConfig, RunContext
...
from mcp_benchmark_sdk.agents.parsers import OpenAIResponseParser
...
| `greater_than` | `gt`, `>` | Numeric greater than |
| `less_than` | `lt`, `<` | Numeric less than |
| `greater_than_equal` | `gte`, `>=` | Greater or equal |
| `less_than_equal` | `lte`, `<=` | Less or equal |
...
@dataclass
class RunResult:
    model: str                          # Model identifier (e.g., "gpt-5-high")
...
|----------|---------------|-------------|------------------|
| **OpenAI** | `gpt-5-high` | 0.1 | High reasoning effort, encrypted reasoning |
| **Anthropic** | `claude-sonnet-4-5` | 1.0 | Extended thinking, 10k token budget |
...
agent = create_agent("gpt-5-high")        # → GPTAgent
...
agent = create_agent("openai:gpt-5-high")
agent = create_agent("anthropic:claude-sonnet-4-5")
```

### Supported Models

**OpenAI:** `gpt-5-high`, `gpt-5`, `gpt-4o`, `gpt-4o-mini`, `o4-mini`  
...
from mcp_benchmark_sdk.agents.core import ClaudeAgent, GPTAgent
from mcp_benchmark_sdk.agents.tasks import Task, Result
from mcp_benchmark_sdk.agents.mcp import MCPConfig
from mcp_benchmark_sdk.agents.runtime import RunContext, RunObserver
from mcp_benchmark_sdk.harness import TestHarness, TestHarnessConfig
from mcp_benchmark_sdk.harness.verifiers import DatabaseVerifier
```
